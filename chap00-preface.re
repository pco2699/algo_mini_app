...
「アルゴリズムとデータ構造を死ぬほど勉強しておけ！」と次のようなサイトや記事、本をやまほど、案内されます。
...
結果的に、受けた会社自体は、すべて落ちてしまったのですが
そこからアルゴリズムの面白さに目覚めて、今でも@<href>{https://leetcode.com, LeetCode}や@<href>{https://atcoder.jp/, AtCoder}をやったりします。

アルゴリズムの勉強をしてて思うのが「アルゴリズムとWebアプリの間に断絶がある」ということです。

アルゴリズムの勉強で作るのは、主に次のようなものです。（実際のLeetCodeを日本語に訳したもの）
