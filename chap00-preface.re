...
はじめまして、たかやま(@pco2699)と申します。
普段は、資産運用のスタートアップでJavaのバックエンドエンジニアとして働いています。
土日はハッカソンに出たり、このようなちょっとしたアプリをつくるような開発本を書いたり
ちょっとした0->1を楽しむのが、趣味です。

== アルゴリズムミニアプリとは

はてブのホッテントリやTwitterでも話題に上がる、@<b>{Webエンジニアにアルゴリズム・データ構造などの知識は必要か？}論。
皆さんも目にしたことがあると思います。この論について、どのような意見をお持ちでしょうか。

私は、@<b>{無くても困らないけど、あると色々助かる、}ものだと思っています。
CS(コンピュータ・サイエンス)と呼ばれる次のような知識は、エンジニアにとっては@<b>{筋肉}です。
無くて困ることはありませんが、あるといろいろ便利です。
...
ただ、Webエンジニアとして働いていると、アルゴリズムがそのまま出てくることはあまり無いです。
簡単なWebページを作る、ということであれば、アルゴリズムの知識なんて全然使いません。

しかも、町の本屋さんやAmazonで売っている「アルゴリズム・データ構造」の本は
ストイックにアルゴリズムやデータ構造が書いてあるので、読む気が失せます。
 ...
 * Facebookの友達検索機能、など、Webサービスの1つの機能だけをアプリとして切り出したもの。
 * ゲーム好きの方なら「メイドインワリオ」のWebサービス版をイメージしてもらうとわかりやすい。

== 本書を書くきっかけ

私がなぜ「アルゴリズム」の本を書こうと思ったか、そのきっかけについて説明します。
自分がアルゴリズムを勉強したタイミングは人生で2回あります。
...
アルゴリズムの勉強をして感じたのが@<b>{アルゴリズムとWebアプリの間に断絶がある}ということです。

アルゴリズムの勉強で作るのは、次のようなものです。(LeetCodeの問題を日本語に訳したもの)

//note[アルゴリズムの問題]{
i番目の配列がi日目の株の値段を表す配列があります。
最大で1回のトランザクション(例: 1個の株を買い、1個の株を売る)が許されている場合、最大の利益を探すアルゴリズムを設計しなさい。

株を買う前に売ることが出来ないことに気をつけること。
//}

このアルゴリズムの問題は非常に有名ですが、入力は配列で、出力は1つの値です。
実際に、コードを書いてみると次のような出力になります。
...
ただ、感じたのが、@<kw>{「アルゴリズムとWebアプリの間で断絶があるのでは無いか」}ということです。

=== アルゴリズムとWebアプリの間にある断絶

「アルゴリズムとWebアプリの間にある断絶」とは、アルゴリズムの勉強で書くコードや成果物が
実際のWeb開発とはかけ離れている、ということです。

そこで、「アルゴリズムの入出力をWebアプリにして、その断絶を無くせないか？」と考えました。

アルゴリズムの教材で使われるC++やPythonは用いず、JavaScriptを用いて説明します。
そして、そのままWebアプリにしてしまうことで、今までに無いアルゴリズムの本ができると考えています。
...
JavaScriptを書いたことが無くても、Python、Ruby、 PHPなどの言語をどれか1つで
簡単なWebシステムやツールを書いたことがある方なら理解できる内容となっています。
...
@<chap>{chap01-install}では、アルゴリズムミニアプリのシステム構成、環境構築方法について説明します。
@<chap>{chap02-search}では、探索のアルゴリズムの内容について説明します。
@<chap>{chap03-search_app}では、探索のアルゴリズムを使ったアルゴリズムミニアプリの開発について説明します。
