...
今回の上記の2つの中の1つ目である@<kw>{効率的に行えるか}について、中でもシンプルな@<kw>{探索} を通して学びたいと思います。
...
//footnote[search][正確にいうと今回学ぶ探索は「リスト探索」といいます。もう一つ探索として対象がグラフの「グラフ探索」がありますが、これは続刊に説明を譲ります。]
...
探索のアルゴリズムは、この数字当ての「Bさん側の数字を言っていく戦略」です。
...
線形探索は、Bさんが「1から100まで一つづつ数字を上げて」言っていく戦略です。
具体的にBさんは次の戦略を取ります。
...
そこで、もう少し効率的な戦略にします。
二分探索の登場です。二分探索は、Bさんが「必ず想定される範囲の真ん中の値を言っていく」戦略です。
...
二分探索は、探索を行うライブラリでも
二分探索で実装されているのが、一般です。@<fn>{citation_needed}
...
たとえば、トランプの色を当てるゲームの場合でも、それぞれの色の間で順番をつけない限り、同じ戦略で当てることは出来ないですよね。
つまり@<b>{探索を行う対象が順番があり、整列されていること}が二分探索では必要です。気をつけてください。(@<img>{binary_note})

//image[binary_note][二分探索の注意点][scale=0.8]{
//}
...
//note[logの復習]{
@<m>{log}は@<kw>{？回 数字を掛け合わせると、その数になる}という？にあてはまる数です。
たとえば@<m>{log_2^8}は@<kw>{？回 2を掛け合わせると、8になる}の？です。2を3回掛け合わせると(@<m>{2^3})=8になるので
？には3がはいります。
//}
