...
今回の上記の2つの中の1つ目である@<kw>{効率的に行えるか}について、中でもシンプルな@<kw>{探索} を通して学びます。
#@# 今回の -> 今回は かの？
...
#@# ちょっと分かりづらかった。探索処理を行う多くのライブラリは
#@# そのアルゴリズムとして二部探索を採用している、ということかな？
...
たとえば@<m>{log_2^8}は@<kw>{x回 2を掛け合わせると、8になる}のxです。2を3回掛け合わせると(@<m>{2^3})=8になるので
xには3がはいります。
//}
...
#@# 表の見方がスッとはわからなかった
#@# 横軸の5、10は探索の対象数で、縦軸が〜と一言あるとスムーズに進めそう
...
実際に、1000で線形探索をすると、1000回の計算量です。それが二分探索だとなんと@<kw>{10}回で済みます。
#@# SUGOI
...
#@# 図や言葉による例が出されながら進むため、全体をとおしてスムーズに読み進める事ができた。