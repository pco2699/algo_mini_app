...
#@# この辺は挙動がイメージできなかった。が、手を動かしながらだと大丈夫な気もする
...
#@# v-radio-groupでラジオボタンの集合を示すタグです、の「で」が一瞬引っかかった。「は」で良いかもしれない
...
まずは、入力内容について簡単なバリデーションをつけてみましょう。
入力内容は@<tt>{Vue}のオートバインディングで、さきほど定義したモデル入っています。
#@# 定義したモデル入っています 「に」抜け？
