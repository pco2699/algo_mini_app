...
アプリは @<href>{https://algo-mini-app.web.app/}にデプロイされているので確認してみてください。
...
├── jest.config.js
├── nuxt.config.js
├── modules // 1. アルゴリズムのロジックを書いておく場所
├── pages // 2. Webのページを置いておく場所
（...省略）
├── plugins
├── static
└── store
...
@<tt>{modules}配下で、アルゴリズムのロジックを記述します。これは、デフォルトでは作成されていません。
そのため、自分で作成しましょう。今後、探索以外のロジックも実装することが予想されるため、@<tt>{modules}の下に@<tt>{search}というディレクトリも作りましょう。
...
次の順番でサンプルアプリのコードを説明します。

　1. @<tt>{modules}配下の探索のロジック
　2. @<tt>{page}配下のWebページ

== 探索のコードを書こう

探索のロジックを@<tt>{modules/search}配下に記載していきましょう。
...
@<tt>{search_util.js}では、@<tt>{ticketGenerator}という、指定された数のチケットの配列を自動で生成するロジックを書いています。
...
@<code>{contents}は、お遊び要素で、指定されている@<tt>{contents}からチケットの内容をランダムで埋め込みます。@<fn>{ticket_contents}
...
@<code>{test(fn)}でテストケースを定義します。 そして@<code>{expected}というチケットの想定配列を手で書いて、
@<code>{ticketGenerator}を動かした結果を比較しています。

@<code>{expect(実際の値).toEqual(期待値)}という形で、実際の値と期待値を比較して同じこと確認します。
...
次のように、PASSと出たらテストが通ったということです。おめでとうございます。
もし通ってなかったらFAILという文字とともに差分が出力されます。差分をもとに原因を特定して修正しましょう。
//cmd{
$ jest
...
クラスにまとめているのは、@<tt>{export}のときに取り扱いやすくするためだけです。それぞれ別の関数にして、@<tt>{export}してしまっても問題ないです。
...
@<tt>{for of}の中で、チケットが探している対象のものか一個ずつ番号/IDを比較しています。
...
このロジックを検証するテストコードを書いてみましょう。ファイル名は@<tt>{searcher.test.js}です。
...
見つかる=@<tt>{null}以外が返ってくる想定のため@<code>{expect.anything()}で@<tt>{null}以外のなにかを期待値として検証します。
...
最初に@<code>{tickets.length}が1以上かどうかを確認しています。0の場合、後で出てくる@<code>{let right = tickets.length - 1}で
マイナスの値になってしまい、誤動作してしまうため、ここでチェックしています。

次に@<tt>{left}と@<tt>{right}という２つの変数を定義しています。二分探索において、非常に重要な変数です。
探索を行う範囲を決めるもので@<tt>{left}が範囲の左端、@<tt>{right}が範囲の右端を示します。
...
左端と右端を足し合わせて2で割ることにより、範囲の真ん中の値を計算しています。
@<code>{Math.floor()}をしている理由は、割り切れない場合、小数となってしまい、配列にアクセスすることが出来ないからです。
@<code>{Math.floor()}で、小数点以下の切り下げを行うことができます。


@<tt>{mid}の値で、配列にアクセスします。@<code>{tickets[mid].id}で@<tt>{mid}に対応するIDを取得できます。
@<code>{tickets[mid].id}が@<tt>{target}の値と一致するかどうかを確認します。
...
こちらは線形探索と内容は同じです。ただし最後の@<tt>{res.count}は、計算量が異なるため注意してください。
一通り、ロジックを書くことができました。
つづいて、画面を作っていきましょう。
...
=== Vuetifyの基本
@<tt>{Vuetify} @<fn>{vuetify}というVue用のUIコンポーネントフレームワークを利用して、画面を作っていきます。
...
さまざまなコンポーネントがあるので、ぜひ@<href>{https://vuetifyjs.com/ja/components/api-explorer, 公式のドキュメント} @<fn>{vuetify_doc}を参照してみてください。

//footnote[vuetify_doc][https://vuetifyjs.com/ja/components/api-explorer]

=== 今回の画面構成
アプリの画面構成について、説明します。シンプルです。
...
//}
ホーム画面を作ります。左にメニューがあり、「探索」を選択すると2.の「探索を行う画面」に移動できます。
この画面と全体のレイアウトについては、ソースコードの説明は省略しますので、気になる方はソースコードを見てください。
...
探索を行うための項目を入力する画面です。この画面を作る方法をメインで解説していきます。

=== 今回のディレクトリ構成

画面を作り始める前に、ディレクトリ構成・ファイル構成を説明します。
...
@<tt>{index.vue}については、説明を割愛しますので、ソースコードを参照してみてください。

Vue/Nuxt.jsは、様々なUIコンポーネントをファイルに分割して自作できるのが特徴です。
そこに拘わらず、シンプルに１つのファイルで画面を作っていきます。

== 探索画面を実際に作ってみよう
探索を行う画面を作っていきましょう。


@<tt>{vue}ファイルをつくるところからです。
まず空のファイルを作ります。
//cmd{
$ cd pages
$ touch search.vue
...
エディタで次の通り@<tt>{vue}ファイルの下地をつくります。
...
<style>
/* ここにCSSでスタイルを記述します。(今回はほぼ使わないので省略) */
</style>
}
//}

ファイルができたので、@<tt>{HTML}と@<tt>{JavaScript}を記載していきましょう。

=== HTML部をつくろう
@<tt>{HTML}部を作りましょう。今回は@<tt>{Vuetify}を用いて作成します。
@<img>{search_screen_app_explained})のイメージを頭に入れて作っていきましょう。
...
==== フォーム
入力する部分=フォームを作ります。
全体を@<tt>{v-form}で囲います。HTMLで@<tt>{form}タグで囲うのと全く同じ要領です。
このアプリはPOSTなどは行わないため、この@<tt>{v-form}タグは省略しても構いません。
...
@<tt>{v-radio-group}はラジオボタンの集合を示すタグです。@<tt>{v-radio}はラジオボタンのそれぞれの項目に対応しています。
...
@<tt>{v-input}は、汎用的なインプットを作るためのコンポーネントです。しかし、今回はエラーメッセージを表示するために使っています。
...
==== 画面を確認してみよう
これで、画面は完成したのでブラウザを立ち上げて画面を確認してみましょう。
次のコマンドで画面を立ち上げることができます。
//cmd{
$ nuxt
//}

問題なく画面が確認できたらOKです。もし問題があったら、エラーメッセージをもとにデバッグしてみてみましょう。
...
モデルの意味は@<table>{model_definition}の通りです。
...
入力内容について簡単なバリデーションをつけてみましょう。
入力内容は@<tt>{Vue}のオートバインディングで、さきほど定義したモデルに入っています。
...
@<tt>{checkForm()}形で、メソッドを書いていけば処理を書いていくことができます。
@<tt>{checkForm()}内では、エラーの内容を管理する@<tt>{errors}という配列を宣言しています。
...
@<tt>{checkForm()}は、HTMLで@<tt>{v-btn}の@<tt>{@click}に紐づけています。
この紐付けにより、ボタンを押すと、@<tt>{checkForm()}が動くようになっています。

@<tt>{@click}は@<code>{checkForm() && execSearch()}となっています。
@<tt>{checkForm()}の結果によって、@<tt>{execSearch()}を動かすかどうかを決めたいからです。

@<tt>{checkForm()}が@<tt>{true}であれば、@<tt>{execSearch()}は動きますし
逆に@<tt>{false}であれば動きません。

==== 探索のロジックを組み込もう
いよいよ作った探索のロジックを組み込んで完成です。
...
動かしてみて問題なく動けば無事にアプリが完成です！

== Firebaseを用いてネットに公開しよう
Firebase Hostingを使って、作ったアプリを公開してみましょう！
実に簡単な方法で公開できます。
...
//terminal[nuxt_generate][Nuxtでのgenerateの様子]{
$ npm run generate
...
//terminal[firebase_deploy][Firebaseへのデプロイ]{
